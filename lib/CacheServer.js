...
var SERVER_STATUS = require('./common').SERVER_STATUS
...
function CacheServer(uri) {
  ...
  this._status  = SERVER_STATUS.DISCONNECTED
...
CacheServer.prototype.close = function () {
  ...
  this._status = SERVER_STATUS.DESTROYED
  this._client.close()
}
...
CacheServer.prototype._scheduleConnect = function () {
  if (this._connectionTimer || this._status == SERVER_STATUS.DESTROYED) return
  ...
  if (!this._connectionIntervalMs) {
    attemptMs = 1
    this._connectionIntervalMs = MIN_BACKOFF
  } else {
    attemptMs = this._connectionIntervalMs
    if (this._connectionIntervalMs < MAX_BACKOFF) {
      this._connectionIntervalMs *= BACKOFF_MULTIPLIER
      if (this._connectionIntervalMs > MAX_BACKOFF) this._connectionIntervalMs = MAX_BACKOFF
    }
  }
...
CacheServer.prototype._onTimeout = function () {
  this._status = SERVER_STATUS.TIMED_OUT
...
CacheServer.prototype._onConnect = function () {
  this._connectionIntervalMs = 0
  this._status = SERVER_STATUS.CONNECTED
