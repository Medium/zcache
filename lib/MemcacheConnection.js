...
MemcacheConnection.prototype.mgetResponseBuilder = function (numItems, defer, responseArr, message) {
  if (message.header.status != memc.constants.status.NO_ERROR || !message.header.bodylen) {
    responseArr.push(undefined)
  } else {
    var bodyBuffer = message.body
    if (this._encoding) bodyBuffer = new Buffer(bodyBuffer.toString(this._encoding), this._encoding)
    responseArr.push(bodyBuffer.toString('utf8'))
  }

  if (responseArr.length === numItems) defer.resolve(responseArr)
}
...
MemcacheConnection.prototype.mget = function (keys) {
  ...
  var defer = Q.defer()
  var responseArr = []
  var responseParser = this.mgetResponseBuilder.bind(this, keys.length, defer, responseArr)

  for (var i = 0; i < keys.length; i++) {
    if (i < keys.length - 1) {
      this._connection.get(keys[i], responseParser)
    } else {
      this._connection.get(keys[i], responseParser)
    }
  }
