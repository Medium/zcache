...
/** @override */
MultiplexingCache.prototype.mget = function (keys, opt_args) {
  var keysToFetch = []
  ...
  if (keysToFetch.length > 0) {
    var mgetPromise = this._mget(keysToFetch, opt_args)
 ...
 * @private
 */
MultiplexingCache.prototype._mget = function (keys, opt_args) {
  var self = this
  var invalidateFn = function (results) {
    self._invalidateKeys(keys)
    return results
  }
  return this._delegate.mget(keys, opt_args)
    .then(invalidateFn)
    .fail(invalidateFn)
