 ...
 * @param {number} timeoutMs The timeout limit by ms.
 ...
 */
function makeNodeResolverWithTimeout(deferred, timeoutMs, timeoutMsg) {
  ...
  return function(err, data) {
    ...
    //if this request finally goes through.
  }
}
...
RedisConnection.prototype.set = function (key, val, maxAgeMs) {
  ...
  this._client.setex(key, Math.floor(maxAgeMs / 1000), val,
      makeNodeResolverWithTimeout(deferred, REQ_TIMEOUT_MS, 'Redis "set" timeout.'))
...
RedisConnection.prototype.mset = function (items, maxAgeMs) {
  ...
  this._client.multi(commands).exec(
      makeNodeResolverWithTimeout(deferred, REQ_TIMEOUT_MS, 'Redis "mset" timeout.'))
...
RedisConnection.prototype.del = function (key) {
  ...
  this._client.del(key,
      makeNodeResolverWithTimeout(deferred, REQ_TIMEOUT_MS, 'Redis "del" timeout.'))
  return deferred.promise
    .fail(warnOnError)
...
RedisConnection.prototype.mget = function (keys) {
  ...
  this._client.mget(keys,
      makeNodeResolverWithTimeout(deferred, REQ_TIMEOUT_MS, 'Redis "mget" timeout.'))
  return deferred.promise
    .then(function (vals) {
...
RedisConnection.prototype.getServerInfo = function () {
  var deferred = Q.defer()
  this._client.info(makeNodeResolverWithTimeout(deferred, REQ_TIMEOUT_MS, 'Redis "info" timeout.'))
  return deferred.promise
