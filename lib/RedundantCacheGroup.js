...
RedundantCacheGroup.prototype.mset = function (items, maxAgeMs) {
  if (!this._isValidTtl(maxAgeMs)) {
    return Q.reject('Invalid TTL passed in to RedundantCacheGroup')
  }

  var promises = []

  var isTtlPrioritized = Array.isArray(maxAgeMs)
  for (var i = 0; i < this._cacheInstances.length; i++) {
    var ttl = isTtlPrioritized ? maxAgeMs[i] : maxAgeMs
    if (this._cacheInstances[i].instance.isAvailable()) promises.push(this._cacheInstances[i].instance.mset(items, ttl))
  }
...
RedundantCacheGroup.prototype.set = function (key, val, maxAgeMs) {
  ...
  for (var i = 0; i < this._cacheInstances.length; i++) {
    var ttl = isTtlPrioritized ? maxAgeMs[i] : maxAgeMs
    if (this._cacheInstances[i].instance.isAvailable()) promises.push(this._cacheInstances[i].instance.set(key, val, ttl))
  }
...
/**
 * Checks if the passed  in ttl is valid, it can be the same value for all the instnaces or can be a list with each entry corresponding
 * to a prioritized cache instance
 * @param  {number|Array}  maxAgeMs
 * @return {boolean}
 */
RedundantCacheGroup.prototype._isValidTtl = function (maxAgeMs) {
  return !Array.isArray(maxAgeMs) || maxAgeMs.length === this._cacheInstances.length
}
